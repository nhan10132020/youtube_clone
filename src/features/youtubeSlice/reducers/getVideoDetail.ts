...
export const getVideoDetails = createAsyncThunk(
  ...
  async (id: string) => {
    const {
      ...
      `${YOUTUBE_API_URL}/videos?key=${API_KEY}&part=snippet&part=statistics&type=video&id=${id}`
    );
...
const parseData = async (item: {
  ...
  id: string;
  statistics: { viewCount: string; likeCount: string,commentCount: number };
}) => {
  const {
    data: {
      items: [
    ...
    `${YOUTUBE_API_URL}/channels?part=snippet&part=statistics&id=${item.snippet.channelId}&key=${API_KEY}`
  );
  return {
    videoId: item.id,
    ...
    commentCount: item.statistics.commentCount?item.statistics.commentCount.toLocaleString():null,
    channelInfo: {
